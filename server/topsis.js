...
function topsis(universities, criteria) {
  ...
  // Only criteria the universities actually differ on can explain anything.
  // Where every university holds the same value, ideal-best equals
  // ideal-worst, so EVERY university sits at distance 0 from the ideal --
  // which would otherwise win "closest to ideal" outright and be reported as
  // their single greatest strength. Library access, held by all six, is not
  // what makes any one of them stand out.
  const discriminating = criteria.map((_, j) => j).filter(j => best[j] !== worst[j]);
  ...
  return universities.map((u, i) => {
    ...
    // Nothing separates these universities at all -> no strength to name,
    // and the UI omits the pill rather than inventing one.
    const bestJ = discriminating.length
      ? discriminating.reduce((a, b) => (distToBest[b] < distToBest[a] ? b : a))
      : -1;
    ...
    const weakCodes = discriminating
      .filter(j => j !== bestJ && distToWorst[j] < distToBest[j])
      .sort((a, b) => distToBest[b] - distToBest[a])
      .slice(0, 3)
      .map(j => criteria[j].code);
    return { ...u, cc, bestCode: bestJ === -1 ? null : (criteria[bestJ]?.code || null), weakCodes };
  }).sort((a, b) => b.cc - a.cc);
}
