...
async function hydrateUniversity(u) {
  ...
  return {
    ...
    // kApplyUrls map in the app, so the seeded universities keep working
    // until their officer sets one.
    applyUrl: (sd.criteria && sd.criteria.applyUrl) || null,
...
module.exports = {
  ...
  async setStudentSuspended(id, suspended, reason) {
    ...
    const p = await getPool();
    await p.query('UPDATE users SET suspended=?, suspend_reason=? WHERE id=?',
      [suspended ? 1 : 0, suspended ? (reason || null) : null, id]);
    const [rows] = await p.query('SELECT email, name FROM users WHERE id=?', [id]);
  ...
  async renameStaffProgramme(uniId, oldName, newName) {
    ...
    d.combos = combos;
    await this._saveStaffData(uniId, d);
    await p.query('UPDATE programmes SET name=? WHERE university_id=? AND name=?', [newName, uniId, oldName]);
    return d;
  },
  ...
  async deleteStaffProgramme(uniId, name) {
    ...
    d.combos = combos;
    await this._saveStaffData(uniId, d);
    await p.query('DELETE FROM programmes WHERE university_id=? AND name=?', [uniId, name]);
    return d;
  },
  ...
  async notAppliedStudents() {
    ...
    const [rows] = await p.query(`
      SELECT u.id, u.name, u.email, COALESCE(u.track,'') AS track,
             COALESCE(u.home_area, u.home, '') AS home,
             lr.university_ids,
             (SELECT COUNT(*) FROM shortlists s WHERE s.user_id = u.id) AS shortlisted
      FROM users u
      LEFT JOIN user_last_ranking lr ON lr.user_id = u.id
      WHERE u.role = 'student'
  ...
  async universityPopularity() {
    const p = await getPool();
    const [apps] = await p.query(
      'SELECT university_id, COUNT(DISTINCT user_id) AS n FROM applications ' +
      'WHERE user_id IS NOT NULL GROUP BY university_id');
    const counts = Object.fromEntries(apps.map(r => [r.university_id, Number(r.n)]));
    const [ap] = await p.query('SELECT COUNT(DISTINCT user_id) AS n FROM applications WHERE user_id IS NOT NULL');
  ...
  async staffCriteriaUsage(uniId) {
    const p = await getPool();
    const [appUsers] = await p.query('SELECT DISTINCT user_id FROM applications WHERE university_id=?', [uniId]);
    const applicantIds = new Set(appUsers.map(r => r.user_id).filter(Boolean));
    if (!applicantIds.size) return [];
    const [rows] = await p.query('SELECT user_id, criteria FROM user_last_ranking');
    const [labelRows] = await p.query('SELECT code, label FROM criteria');
    const labelByCode = Object.fromEntries(labelRows.map(r => [r.code, r.label]));
    const counts = {};
    for (const row of rows) {
  ...
  async staffCombosReached(uniId) {
    const p = await getPool();
    const [lr] = await p.query('SELECT user_id, university_ids FROM user_last_ranking');
    const memberIds = lr.filter(r => {
      try { return JSON.parse(r.university_ids || '[]').some(u => u.id === uniId); } catch { return false; }
    }).map(r => r.user_id).filter(Boolean);
    if (!memberIds.length) return [];
    const [users] = await p.query('SELECT track FROM users WHERE id IN (?)', [memberIds]);
    const counts = {};
